module.exports = {
  ...
  function init() {
    ...
    const RUST_TOKENS = {
      ...
      string: /"[^"\n]*"|b?'.'/,
      lifetime: /'\w+(?!')/,
      kw: /\b(static|move|super|type|trait|where|self|impl|true|false|extern|crate|fn|match|in|if|else|while|for|loop|pub|let|return|break|continue|mut|const|ref|struct|enum|use|mod|as|unsafe|dyn)\b/,
      macro: /[a-zA-Z_]+!(?=[(\\[{])/,
      comment: /\/\/.*/,
      prompt: /\$/,
      text: /<\w+[^<]*>|<\/\w+>/,
    ...
    const LANG_TOKENS = {
      ...
      string: /(?<!=)"[^"\n]*"(?!>)/,
      kw: /\b(fun|val|is|interface|void|in|if|else|while|for|let|return|break|continue|const|use|def|private|static|true|false)\b/,
      comment: /\/\/.*/,
      macro: /#\w+ .*/,
      text: /<\w+[^<]*>|<\/\w+>/,
    }
